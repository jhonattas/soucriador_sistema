...
export default {
  ...
  methods: {
    fetchProductsData: function (id) {
      this.equipments = []
      var url = process.env.API_URL + 'products/' + id
  ...
  watch: {
    '$route.params.id': function (id) {
      this.fetchProductsData(id)
    }
  },
  mounted: function () {
    try {
      var title = 'UniversalEquipamento'
      document.title = title
    } catch (error) {
      console.log('nao foi possivel alterar titulo')
    }
    this.fetchProductsData(this.$route.params.id)
  }
}
</script>
